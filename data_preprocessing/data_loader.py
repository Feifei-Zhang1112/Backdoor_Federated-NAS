...
from data_preprocessing.datasets import TinyImageNet_truncated
logging.basicConfig()
...
def load_cifar10_data(datadir):
    ...
    cifar10_train_ds = CIFAR10_truncated(datadir, train=True, download=True, transform=train_transform) #$$$$ download=True
    cifar10_test_ds = CIFAR10_truncated(datadir, train=False, download=True, transform=test_transform) #$$$$ download=True
    print(len(cifar10_train_ds))
...
def _data_transforms_tinyimagenet():
    TINY_IMAGENET_MEAN = [0.485, 0.456, 0.406]  # These are standard values for ImageNet, might need adjustments
    TINY_IMAGENET_STD = [0.229, 0.224, 0.225]  # These are standard values for ImageNet, might need adjustments

    train_transform = transforms.Compose([
        transforms.RandomResizedCrop(32),  # 修改裁剪大小为32
        transforms.RandomHorizontalFlip(),
        transforms.ToTensor(),
...
def load_tinyimagenet_data(datadir):
    train_transform, test_transform = _data_transforms_tinyimagenet()

    tinyimagenet_train_ds = TinyImageNet_truncated(datadir, train=True, download=True, transform=train_transform)
    tinyimagenet_test_ds = TinyImageNet_truncated(datadir, train=False, download=True, transform=test_transform)

    print(len(tinyimagenet_train_ds))

    X_train, y_train = np.array(tinyimagenet_train_ds.data), np.array(tinyimagenet_train_ds.target)
    X_test, y_test = np.array(tinyimagenet_test_ds.data), np.array(tinyimagenet_test_ds.target)
...
def get_dataloader(dataset, datadir, train_bs, test_bs, dataidxs=None):
    if dataset in ('mnist', 'cifar10', 'tiny'):
        if dataset == 'mnist':
