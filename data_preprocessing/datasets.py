...
class TinyImageNet_truncated(data.Dataset):
    ...
    def __build_truncated_dataset__(self):
        ...
        selected_wnids = all_wnids[:self.num_classes]

        if self.train:
            dataset = ImageFolder(data_path)
            selected_samples = [(img_path, label) for img_path, label in dataset.samples if
                            dataset.classes[label] in selected_wnids]
        else:
            val_annotations_path = os.path.join(self.root, 'val', 'val_annotations.txt')
            with open(val_annotations_path, 'r') as f:
                val_annotations = [line.split('\t') for line in f.readlines()]

            img_path_to_wnid = {os.path.join(data_path, 'images', line[0]): line[1] for line in val_annotations}

            selected_samples = [(img_path, selected_wnids.index(wnid)) for img_path, wnid in img_path_to_wnid.items() if
                                wnid in selected_wnids]
        data = [Image.open(img_path).convert("RGB") for img_path, _ in selected_samples]
        target = [target for _, target in selected_samples]
