...
OPS = {
    'none': lambda C, stride, affine: Zero(stride),
    'ghost_conv_3x3': lambda C, stride, affine: GhostModule(C, C, 0.5, 1, stride, 3),
    'ghost_conv_7x7': lambda C, stride, affine: GhostModule(C, C, 0.1, 1, stride, 7),
    'fire': lambda C, stride, affine: Fire(C, C, stride, 0.875, 4/3),
    'sep_conv_7x7': lambda C, stride, affine: SepConv(C, C, 7, stride, 3, 4, affine=affine),
    'relu_conv_7x7': lambda C, stride, affine: ReLUConvBN(C, C, 7, stride, 3, affine=affine),
    'senet_2': lambda C, stride, affine: SENet(C, C, stride, 2),
    'senet_4': lambda C, stride, affine: SENet(C, C, stride, 4),
    'skip_connect': lambda C, stride, affine: Identity() if stride == 1 else GhostModule(C, C, 0.1, 1, stride, 5),
    'conv_7x1_1x7': lambda C, stride, affine: nn.Sequential(
        nn.ReLU(inplace=False),
        nn.Conv2d(C, C, (1, 7), stride=(1, stride), padding=(0, 3), bias=False),
        nn.Conv2d(C, C, (7, 1), stride=(stride, 1), padding=(3, 0), bias=False),
...
OPS = {
    'none': lambda C, stride, affine: Zero(stride),
    'skip_connect': lambda C, stride, affine: Identity() if stride == 1 else DepthwiseSeparableConv2d(C, C, 3, stride, 2, 2),
    'group_3x3_4': lambda C, stride, affine: GroupedConv2d(C, C, 3, stride, 1, 4),
    'group_3x3_2': lambda C, stride, affine: GroupedConv2d(C, C, 3, stride, 1, 2),
    'group_7x7_32': lambda C, stride, affine: GroupedConv2d(C, C, 7, stride, 3, 32),
    'group_5x5_8': lambda C, stride, affine: GroupedConv2d(C, C, 5, stride, 2, 8),
    'group_7x7_16': lambda C, stride, affine: GroupedConv2d(C, C, 7, stride, 3, 16),
    'group_5x5_32': lambda C, stride, affine: GroupedConv2d(C, C, 5, stride, 2, 32),
    'sep_conv_7x7': lambda C, stride, affine: SepConv(C, C, 7, stride, 3, 4, affine=affine),
    'conv_7x1_1x7': lambda C, stride, affine: nn.Sequential(
        nn.ReLU(inplace=False),
        nn.Conv2d(C, C, (1, 7), stride=(1, stride), padding=(0, 3), bias=False),
...
class GhostModule(nn.Module):
    def __init__(self, in_channels, fixed_ghost_channels, ratio, group, stride=1, kernel_size=1, dw_size=3, relu=True):
        super(GhostModule, self).__init__()
        real_channels = int(fixed_ghost_channels * ratio)
        self.oup = fixed_ghost_channels
        init_channels = fixed_ghost_channels - real_channels
        new_channels = real_channels
        ...
        self.cheap_operation = nn.Sequential(
            nn.Conv2d(init_channels, new_channels, dw_size, 1, dw_size // 2, groups=group, bias=False),
...
class Fire(nn.Module):
    def __init__(self, inplanes, out_channels, stride, ratio_1, ratio_2):
        super(Fire, self).__init__()
        expand1x1_planes = int(out_channels * ratio_1)
        expand3x3_planes = out_channels - expand1x1_planes
        squeeze_planes = int(inplanes * ratio_2)
