...
class Architect(object):
    ...
    def step_back(self, batch_back, batch_back_search, attacker, lambda_train_regularizer):
        self.optimizer.zero_grad()

        loss_adv_train = attacker.compute_blind_loss(self.model, self.criterion, batch_back, True, train=False)

        arch_parameters = self.model.arch_parameters()
        grads_alpha_with_train_dataset = torch.autograd.grad(loss_adv_train, arch_parameters)

        self.optimizer.zero_grad()
        loss_adv = attacker.compute_blind_loss(self.model, self.criterion, batch_back_search, True, train=False)

        arch_parameters = self.model.arch_parameters()
        grads_alpha_with_val_dataset = torch.autograd.grad(loss_adv, arch_parameters)
    ...
    def _backward_step(self, input_valid, target_valid):
        logits = self.model(input_valid)
        target_valid = torch.Tensor(target_valid.cpu().numpy()).long()
        target_valid = target_valid.to("cuda:0")
