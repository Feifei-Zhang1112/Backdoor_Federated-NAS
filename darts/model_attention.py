...
from darts.operations_attention import *
...
def layer2module(model, layer: str):
    ...
    print(parts)
    if 'stem' in parts:
        return '.'.join(parts[:-1])  # Returning 'stem.0'
        ...
        if '_ops' in op_name and 'op' in op_name2:
            ...
            if additional_fields == "":
                result = f"cells.{cell_index}.{op_name}.{op_index}.op."

            print(f"Result: {result}")  # Debug print
            return result
        elif '_ops' in op_name and 'op' not in op_name2:
            op_index = parts[3]  # The specific index
            op_part = parts[4]  # The specific part of the operation, e.g., 'expand3x3'
            additional_fields = ".".join(parts[5:]) if len(parts) > 5 else ""
            result = f"cells.{cell_index}.{op_name}.{op_index}.{op_part}.{additional_fields}"
            return result.rstrip(".")
...
class NetworkATTENTION(nn.Module):

    def __init__(self, C, num_classes, layers, auxiliary, genotype):
        super(NetworkATTENTION, self).__init__()
        self._layers = layers
    ...
    def features(self, input): # possible error
