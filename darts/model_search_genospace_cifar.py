...
from darts.genotypes import PRIMITIVES_GENO, Genotype
from darts.operations_genospace_cifar import OPS, FactorizedReduce, ReLUConvBN
...
class MixedOp(nn.Module):
    ...
    def __init__(self, C, stride):
        ...
        for idx, primitive in enumerate(PRIMITIVES_GENO):
            # for primitive in PRIMITIVES:
            op = OPS[primitive](C, stride, False)
            if 'pool' in primitive:
                op = nn.Sequential(op, nn.BatchNorm2d(C, affine=False))
            self._ops.append(op)
            self.primitive_to_idx[primitive] = idx  # search
...
class Network_GENO_CIFAR(nn.Module):

    def __init__(self, C, num_classes, layers, criterion, device, steps=4, multiplier=4, stem_multiplier=3):
        super(Network_GENO_CIFAR, self).__init__()
        print(Network_GENO_CIFAR)
        self._C = C
        self._num_classes = num_classes
        self._layers = layers
        self._criterion = criterion
    ...
    def new(self):
        model_new = Network_GENO_CIFAR(self._C, self._num_classes, self._layers, self._criterion, self.device).to(self.device)
    ...
    def increase_weight_for_path(self, path, increment=1.0):
        ...
        for (op_name, prev_node) in path:
            ...
            op_idx = PRIMITIVES_GENO.index(op_name)
        ...
        def _parse(weights):
            ...
            for i in range(self._steps):
                ...
                edges = sorted(range(i + 2),
                               key=lambda x: -max(W[x][k] for k in range(len(W[x])) if k != PRIMITIVES_GENO.index('none')))[
                        :2]
                for j in edges:
                    k_best = None
                    for k in range(len(W[j])):
                        if k != PRIMITIVES_GENO.index('none'):
                            if k_best is None or W[j][k] > W[j][k_best]:
                                k_best = k

                    if _isCNNStructure(k_best):
                        cnn_structure_count += 1
                    gene.append((PRIMITIVES_GENO[k_best], j))
                start = end
                n += 1
