...
from models.extractor_resnet import FeatureExtractor
...
def get_neuron_weight_names(model: Model):
    ...
    #    print("-------------------------")
    #    print(neuron_targets)
    #    print("-------------------------")
    return neuron_targets


class Attack_F3BA_resnet:
    params: Params
    ...
    def trigger_loss(self, model, backdoor_inputs, clean_inputs, pattern, grads=True):
        ...
        backdoor_inputs, clean_inputs, pattern = ensure_tensors_on_current_device(backdoor_inputs, clean_inputs,
                                                                                  pattern)
    ...
    def compute_blind_loss(self, model, criterion, batch, attack, train=False):
        ...
        if len(loss_tasks) == 1:
            ...
            loss_values, grads = compute_all_losses_and_grads(
                loss_tasks,
                self, model, criterion, batch, batch_back, trian=train, compute_grad=False)
    ...
    def search_candidate_weights(self, model: Model, proportion=0.2):
        ...
        candidate_weights = OrderedDict()  # ----------------------》有序字典
    ...
    def calculate_feature_difference(self, raw_model, new_model, task, loader):
        ...
        if isinstance(raw_model, ResNet) and isinstance(new_model, ResNet):
    ...
    def optimize_backdoor_trigger(self, model: Model, candidate_weights, task, loader):
        ...
        for epoch in range(2):
            ...
            for i, data in enumerate(loader):
                ...
                loss, grads = self.trigger_loss(model, backdoor_batch.inputs, clean_batch.inputs, pattern, grads=True)
    ...
    def inject_handcrafted_neurons(self, model, candidate_weights, task, diff, loader):
        ...
        n_labels = -1
        # if isinstance(task, CifarFedTask):
        #     n_labels = 10
