...
class MGDASolver:
    ...
    def _min_norm_2d(vecs: list, dps):
        ...
        for i in range(len(vecs)):
            if vecs[i] is None:
                continue  # Skip if the current vector is None
            for j in range(i + 1, len(vecs)):
                ...
                if (i, j) not in dps:
                    ...
                    for k in range(len(vecs[i])):
                        if vecs[i][k] is None or vecs[j][k] is None:
                            continue  # Skip if the current vector component is None
    ...
    def get_scales(cls, grads, losses, normalization_type, tasks):
        ...
        for t in tasks:
            ...
            for gr_i in range(len(grads[t])):
                if grads[t][gr_i] is None:
                    continue
                grads[t][gr_i] = grads[t][gr_i] / (gn[t] + 1e-5)
        sol, min_norm = cls.find_min_norm_element([grads[t] for t in tasks if grads[t] is not None])
