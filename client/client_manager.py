...
class ClientMananger(Observer):
    ...
    def __handle_msg_client_receive_config(self, msg_params):
        ...
        # batch_idxs = msg_params.get(MPIMessage.MSG_ARG_KEY_BATCH_IDXS)  # adaptive_distillation
        # syn = msg_params.get(MPIMessage.MSG_ARG_KEY_SYN)
        if process_id != 0:
            return
        ...
        if self.args.stage == "search":
            ...
            # self.trainer.batch_idxs = batch_idxs  # adaptive_distillation
    ...
    def __handle_msg_client_receive_model_from_server(self, msg_params):
        ...
        arch_params = msg_params.get(MPIMessage.MSG_ARG_KEY_ARCH_PARAMS)
        genotype = msg_params.get(MPIMessage.MSG_ARG_KEY_GENOTYPE)
        syn = msg_params.get(MPIMessage.MSG_ARG_KEY_SYN)


        if process_id != 0:
            return
        # if self.rank in participated_clients:
        # print(self.rank in participated_clients)
        # if self.trainer.attack is True and self.args.attack_type != 'constrain':
        #     self.trainer.update_syn(syn)
        self.trainer.update_model(model_params)
        if self.args.stage == "search":
                self.trainer.genotype = genotype
                self.trainer.update_arch(arch_params)
               
        self.round_idx += 1
        # if self.rank in participated_clients:
        self.trainer.round = self.round_idx
        self.__train()
    ...
    def __train(self):
        ...
        if self.args.stage == "search":
            weights, alphas, local_sample_num, train_acc, train_loss, logit = self.trainer.search()

        else:
            weights, local_sample_num, train_acc, train_loss, logit = self.trainer.train()
            alphas = []
        ...
        if self.trainer.attack is True and self.args.attack_type != 'constrain':
            syn = self.trainer.syn                   # syn for make_backdoor_batch
        else:
            syn = None
        
        self.__send_msg_fedavg_send_model_to_server(weights, alphas, local_sample_num, train_acc, train_loss, logit, syn)
    ...
    def __send_msg_fedavg_send_model_to_server(self, weights, alphas, local_sample_num, valid_acc, valid_loss, logit, syn):
        msg = MPIMessage()
        ...
        msg.add(MPIMessage.MSG_ARG_KEY_LOGIT, logit)  # adaptive_distillation
        msg.add(MPIMessage.MSG_ARG_KEY_SYN, syn)  # syn for make_backdoor_batch
